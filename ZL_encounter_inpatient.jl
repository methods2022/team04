...
function extract_inpatient(dirt)
    ...
    output_file_name = "inpatient_covid_cohort.txt"
    ...
    println(output_file,"patient|encounter_id|start|stop|duration|ICU")
    ...
    for line in readlines(input_file)
        ...
        if line_count == 1
            ...
            global encounter = findfirst(isequal("Id"),field_array)
            continue
        end
        ...
        encounter_id = line_part_array[encounter]
        ...
        if proc_code == "1505002" && reason =="840539006"
            duration_total += duration
            covid_count += 1
            println(output_file,"$patient|$encounter_id|$start|$stop|$duration|0")
        end
    ...
    close(output_file)
    println(" ")
    #println("Total COVID patient: $covid_count \t Total duration: $duration_total \t Average Duration: $duration_avg")
